...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
uint32_t therm_array[100];		// array values to store multiple readings
uint32_t die_array[100];
...
int main(void)
{
  ...
  MX_ADC3_Init();
  /* USER CODE BEGIN 2 */
  HAL_ADC_Start_DMA(&hadc3, therm_array, 100);
  HAL_ADC_Start_DMA(&hadc1, die_array, 100);
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  therm_value = 0;
	  // continuously takes 100 samples
	  for (int x = 0; x < 100; x++)
	  {
		  therm_value += therm_array[x];
	  }
	  ...
	  thermistor_tempC = (1/((log(((10000.0 * therm_value) / (4095.0 - therm_value))/104000.0)/3950.0) + (1 / (273.15 + 25.000)))) - 273.15;

	  printf("Therm temp: %.1f\n", thermistor_tempC);

	  die_value = 0;

	  for (int i = 0; i < 100; i++)
	  {
		  die_value += die_array[i];
	  }
	  // samples are averaged
	  die_value /= 100;

	  /* Disensor_value is divided by (Vref * num of ADC samples)
	  Vref - 3.0 V, num of ADC samples - (2^(n-1)) */
	  V_sense = (die_value * (3.0 / 4095));
	  // 0.76 - voltage at 25 degrees / (25 mV) - average slope of the temp vs VSense curve
	  diesensor_tempC = ((V_sense - 0.76)/ 0.025) + 25;

	  printf("Die temp: %.1f\n", diesensor_tempC);

	 // toggle orange LED at a rate of 0.5 seconds
